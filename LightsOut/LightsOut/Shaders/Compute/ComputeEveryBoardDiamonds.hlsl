...
void main(uint3 DTid: SV_DispatchThreadID)
{
	...
	if((DTid.x % gCellSize) && (DTid.y % gCellSize)) //Inside the cell
	{
		...
		uint LTCCandidate = (cellValue == leftTopPartValue)     * cellValue;
		uint RTCCandidate = (cellValue == rightTopPartValue)    * cellValue;
		uint RBCCandidate = (cellValue == rightBottomPartValue) * cellValue;
		uint LBCCandidate = (cellValue == leftBottomPartValue)  * cellValue;
		...
		float enablePower = cellPower * insideDiamond + LTPower * insideTopLeft + RTPower * insideTopRight + RBPower * insideBottomRight + LBPower * insideBottomLeft;
		...
		if(gFlags & FLAG_SHOW_SOLUTION) //Show the solution
		{
			...
			uint LTCSolutionCandidate = (solutionValue == leftTopPartSolved)     * solutionValue;
			uint RTCSolutionCandidate = (solutionValue == rightTopPartSolved)    * solutionValue;
			uint RBCSolutionCandidate = (solutionValue == rightBottomPartSolved) * solutionValue;
			uint LBCSolutionCandidate = (solutionValue == leftBottomPartSolved)  * solutionValue;
			...
			float solvedPower = solutionPower * insideDiamond + LTSolutionPower * insideTopLeft + RTSolutionPower * insideTopRight + RBSolutionPower * insideBottomRight + LBSolutionPower * insideBottomLeft;
		...
		else if(gFlags & FLAG_SHOW_STABILITY)
		{
			...
			uint LTCStabilityCandidate = (stableValue == leftTopPartStable)     * stableValue;
			uint RTCStabilityCandidate = (stableValue == rightTopPartStable)    * stableValue;
			uint RBCStabilityCandidate = (stableValue == rightBottomPartStable) * stableValue;
			uint LBCStabilityCandidate = (stableValue == leftBottomPartStable)  * stableValue;
			...
			float stablePower = stabilityPower * insideDiamond + LTStabilityPower * insideTopLeft + RTStabilityPower * insideTopRight + RBStabilityPower * insideBottomRight + LBStabilityPower * insideBottomLeft;
