...
void main(uint3 DTid: SV_DispatchThreadID)
{
	...
	if((DTid.x % gCellSize) && (DTid.y % gCellSize)) //Inside the cell
	{
		...
		if(gSolutionVisible || cellNumber.x == hintTurnX && cellNumber.x == hintTurnY) //We are showing the solution
		{
			...
			bool right2PartSolved      = cellNumber.x < iFieldSize - 2                                  && IsCellActivatedSolution(cellNumber + int2( 2,  0));
			bool top2PartSolved        =                                  cellNumber.y > 1              && IsCellActivatedSolution(cellNumber + int2( 0, -2));
			bool bottom2PartSolved     =                                  cellNumber.y < iFieldSize - 2 && IsCellActivatedSolution(cellNumber + int2( 0,  2));
